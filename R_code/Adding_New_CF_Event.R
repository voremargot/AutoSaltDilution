...
for (x in c(1:nrow(New_Events))){
  ...
  if (New_Events[x,'name']== 'WTS844-20210401-Mid-1.xlsx' | New_Events[x,'name']== 'WTS844-20210401-Mid-2.xlsx'  |
      New_Events[x,'name']=='WTS626-20210401-Mid-1.xlsx' | New_Events[x,'name']== 'WTS626-20210401-Mid-2.xlsx'){
    next()
  }
...
# CF_Summary_cut=  subset(CF_Summary,select=-c(Num))
# 
...
# for (x in c(1:nrow(Events_added))){
...
# for (x in c(1:nrow(Sensor_Summary))){
#   Number= Sensor_Summary[R,'Num']
